...
namespace CWEBF
{
    public partial class CWEBFForm : Form
    {
        ...
        {
            get => new() { Clusters = classes.SubNodes<TreeNodeComment>().Select(n => new ClusterCWEs { Name = n.Name, CWEs = n.SubNodes.Select(s => new ClassCWEs { Class = s.Name, ShowCWEsText = s.Comment }).ToArray() }).ToArray() };
            //xxx color
        ...
        private void OnFileSave(object sender, EventArgs e)
        {
            try
            {
                ...
                using (var xmlReader = XmlReader.Create(Path.Combine(solutionDir, @"XSLT/CWEBF.xslt")))
                ...
                using (var reader = XmlReader.Create(Path.Combine(solutionDir, @"XML\cwec_v4.6.xml")))
                using (var writer = XmlWriter.Create(Path.Combine(solutionDir, @"XML\CWEBF.xml")))
                    xslt.Transform(reader, null, writer, new XmlUrlResolver());
                //    xslt.Transform(reader, x, writer, new XmlUrlResolver());
